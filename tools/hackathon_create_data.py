...
if __name__ == '__main__':
    ...
    """
    try:
        location_tmp = LocationSvc.create('HackCEWIT', user.id, description="This is default house")
    except AlreadyExistsError as error:
        print(error)
        ...
        try:
            house = LocationSvc.get(user.id, name='HackCEWIT')
        except NoRecordError as error:
            print(error)
    ...
    try:
        location_tmp = []
        for idx in range(1):
            location_tmp.extend(LocationSvc.create('main_room' , user.id,
                                                house_id=house.id,
    ...
    try:
        ...
        try:
            device = DeviceSvc.get(user.id, name='RPi_1')
